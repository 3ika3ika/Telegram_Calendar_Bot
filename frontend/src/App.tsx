...
function App() {
  ...
  const authenticate = useCallback(async () => {
    try {
      ...
      // Auto-detect and update user's timezone if not set or set to UTC
      const detectedTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone
      if (userData.timezone === 'UTC' || !userData.timezone) {
        try {
          await apiClient.updateUser(detectedTimezone)
  ...
  useEffect(() => {
    ...
    if (isDevMode && !hasValidInitData) {
      ...
      const mockInitData = `user=%7B%22id%22%3A1478409325%2C%22first_name%22%3A%22ZIKA%22%2C%22last_name%22%3A%22%22%2C%22username%22%3A%22zk3kt%22%2C%22language_code%22%3A%22en%22%7D&auth_date=${Math.floor(Date.now() / 1000)}&hash=dev_mode_hash`
